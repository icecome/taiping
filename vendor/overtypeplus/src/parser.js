  ...
  static buildTree(text) {
    ...
    while (i < lines.length) {
      ...
      if (fence) {
        ...
        children.push({ type: 'code', lang, text: buf.join('\n') });
        continue;
      }

      // horizontal rule
      if (line.trim().match(hrLine)) { children.push({ type: 'hr', line: i }); i++; continue; }
      ...
      if (quoteLine.test(line)) {
        ...
        const text = q.join('\n');
        children.push({ type: 'blockquote', text, inline: this.parseInlineSpans(text) });
        continue;
      }

      // table (header + align row + cells)
      ...
      // list (consecutive list-item lines → one list node)
      if (listItemLine.test(line)) {
        ...
        while (i < lines.length && listItemLine.test(lines[i])) {
          ...
          let content = li[3];
          if (isTask) {
            const t = lines[i].match(taskLine);
            items.push({ ordered, marker: li[2], task: true, checked: t[3].toLowerCase() === 'x', text: t[5], inline: this.parseInlineSpans(t[5]) });
          } else {
      ...
      while (i < lines.length && lines[i].trim() !== '' &&
             !headerLine.test(lines[i]) && !listItemLine.test(lines[i]) &&
             !quoteLine.test(lines[i]) && !hrLine.test(lines[i].trim()) &&
             !pipeCell.test(lines[i]) && !/^(\s*)```/.test(lines[i])) {
        buf.push(lines[i]); i++;
      }
      const text = buf.join('\n');
      children.push({ type: 'paragraph', text, inline: this.parseInlineSpans(text) });
    }
